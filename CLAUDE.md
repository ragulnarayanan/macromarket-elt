...
- **Explain everything before coding.** Rana is learning Azure services (ADLS Gen2, ADF, Databricks, Key Vault) for the first time. Treat each phase as a teaching session. Explain WHAT you're building, WHY it exists in the architecture, and HOW it connects to other components.
...
- **Cloud:** Azure (ADLS Gen2, Data Factory, Databricks, Key Vault, DevOps)
- **Warehouse:** Snowflake (on Azure, East US 2)
- **Transformation:** dbt Core (dbt-snowflake adapter)
- **NLP Enrichment:** Azure Databricks (PySpark + FinBERT)
...
Medallion: Bronze (raw VARIANT) → Silver (staged + intermediate) → Gold (facts + dims + sentiment)
...
- `databricks/` — Sentiment enrichment notebook
...
5. Databricks sentiment enrichment
6. MCP Server
7. Azure Data Factory orchestration
8. Streamlit Dashboard
9. CI/CD + README + polish
...
- Snowflake RBAC: LOADER (Bronze write), TRANSFORMER (dbt + Databricks), REPORTER (MCP + Streamlit).
...
- Databricks writes to `MACROMARKET.GOLD.databricks_sentiment_output`, dbt reads it in `fct_sentiment_enriched.sql`.
